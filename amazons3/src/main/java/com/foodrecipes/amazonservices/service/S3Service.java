...
public class S3Service {
	...
	@Value("${aws.bucket.recipe}")
	private String bucketRecipeName;
	...
	public ResultResponse uploadRecipe(MultipartFile file) {
        MetadataResponse metadataResponse = new MetadataResponse();
        ResultResponse response = new ResultResponse();
        File fileSave = null;
        try {
            ...
            s3Client.putObject(new PutObjectRequest(bucketRecipeName, uniqueFilename, fileSave));
	...
	public ResultResponse deleteRecipe(String file) {
		MetadataResponse metadataResponse = new MetadataResponse();
		ResultResponse response = new ResultResponse();
		if(!file.equals(Constants.DEFAULT_PROFILE_IMAGE)) {
		...
		try {
			...
			s3Client.deleteObject(bucketRecipeName, file);
